...
#             option 1: override eams_probN.cfg INITIAL_TEMPERATURE
            if self.INITIAL_TEMPERATURE_MODE == '1':
                logging.debug("Initial OAT: %s" %(self.OAT.get(datetime.strptime(self.SCHEDULING_START_DATETIME, "%Y-%m-%d %H:%M"))))
...
#             else:
    ...
    def _populateWorkingTimeslot(self):
        """Populate binary indicator list which denotes if the timeslot is in standard working hour """  
        self.WH = []        
        s = datetime.strptime("09:00", "%H:%M")
        e = datetime.strptime("18:00", "%H:%M")
     
        for _, v in self.TS.iteritems():
            if (v.time() >= e.time() or v.time() < s.time()):
                self.WH.append(0)  
            else:
                self.WH.append(1)        
        logging.debug("Standard Working Hours:\n%s" %(self.WH))
...
#                 logging.debug("sidx=%s eidx=%s d=%d" %(sidx, eidx-1, int(self.ML[i].Duration)))
    ...
    def _populateMeetingsWithSimilarAttendees(self):
        ...
        for mx in xrange(len(self.ML)):
            self.MOM.append([])
            # TODO: use set.intersection instead!
...
#                     print tmp_mtype[y].MCA == uniq_mca
        ...
        logging.debug("%s" %new_mtype)   
        logging.debug("Number of new MTYPE:%d" %len(new_mtype))
        for i in xrange(len(new_mtype)):
            print new_mtype[i]
        print "Num MTYPE:", len(new_mtype)
...
#==================================================================    
    ...
    def readProblemInstance(self, filename):    
        # Configure log file
        fn = filename.replace('/', '_')
        fn = fn.replace('.', '_')
        fn = 'Output\EAMS_' + fn + '_' + datetime.now().strftime('%Y_%m_%d_%H_%M_%S_%f')
