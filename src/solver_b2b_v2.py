...
class Solver:
    ...
    def _initializeModel(self):
        ...
        self.model.setParam(GRB.Param.TimeLimit, 60*2) # 60s x  N minutes
...
#         self._createQuadMinRoomObjective()
...
#---------------------                
    ...
    def _createBAV_y_LD(self):
        ...
        for l in xrange(self.NUM_ROOM):
            ...
            for d in xrange(5):   #TODO: hard-coded
                name = ['BAV_y_LD', str(l), str(d)]
                name = '_'.join(name)         
                self.BAV_y_LD[l].append(self.model.addVar(vtype=GRB.BINARY, name=name))
    ...
    def _createCSTR_MinRoom(self):
        ...
        for l in xrange(self.NUM_ROOM):      
            ...
            for k in xrange(self.NUM_SLOT):
                for m in xrange(self.NUM_MEETING):              
                    for dk, [om, ol, ok] in self.BDV_x_MLK_Dict.iteritems():    # Get offset of m & k for each l from BDV_MLK                    
                        if dk == (m,l,k):
                            yk = self._getDayForSlotIdx(k) 
                            lcstr = self.BDV_x_MLK[om][ol][ok]
                            rcstr = self.BAV_y_LD[l][yk] 
    ...
    def _createQuadMinRoomObjective(self):
        """Create an objective to minimize number of room used """
        ...
        for l in xrange(self.NUM_ROOM):      
            ...
            for k in xrange(self.NUM_SLOT):
                for m in xrange(self.NUM_MEETING):              
                    for dk, [om, ol, ok] in self.BDV_x_MLK_Dict.iteritems():    # Get offset of m & k for each l from BDV_MLK                    
                        if dk == (m,l,k):
                            lobj += self.BDV_x_MLK[om][ol][ok]
#                             print dk , "is located at BDV_x_MLK =", [om, ol, ok]
...
#                         print int(self.BDV_x_MLK[m][l][k].x),
                        ...
                        logging.info("Meeting %d [%s]| Location %d [%s] | Start Slot %s [%s]" %(m, self.EAMS.ML[m].Key, l, self.EAMS.RL[fl], fk, self.EAMS.TS.get(fk)))
#                         break                
            print ""
        print ""
        
    def createFixedMeetingSchedule(self, caseid):
        
        print "Creating Fixed Schedule .... "  
        sche = []
        self.mdesc = namedtuple('Meeting_Sche_Desc', 'Key Start End Duration Room Attendees')
        ...
        for m in xrange(self.NUM_MEETING):
            for l in xrange(len(self.BDV_x_MLK[m])):
                for k in xrange(len(self.BDV_x_MLK[m][l])):
                    ...
                    if val > self.EPSILON:
                        [_,fl,fk] = self._getBDV_x_MLK([m,l,k])
                        sche.append(self.mdesc(self.EAMS.ML[m].Key, 
                                               self.EAMS.TS.get(fk),
                                               self.EAMS.TS.get(fk+self.EAMS.ML[m].Duration), 
                                               self.EAMS.ML[m].Duration, 
                                               self.EAMS.RL[fl], 
                                               self.EAMS.ML[m].Attendees))
                  
        print caseid + "_f_b2b"
        print sche
        
        directory = "Data\\Meetings\\" 
        fname = directory + caseid + "_f_b2b.cfg"        
        f = open(fname,'w')
        
        for i in xrange(len(sche)): 
            f.write('[' + str(sche[i].Key) + ']\n')
            f.write('Attendees=' + ", ".join( repr(int(e)) for e in sche[i].Attendees) + "\n")
            f.write('Duration=' + str(sche[i].Duration) + "\n")
            f.write('Preferred_Room=' + str(sche[i].Room) + "\n")
            
            # Fixed Schedule, so, only 1 timewindow    
            f.write("\t[[W0]]\n")
            f.write("\tStart="+ sche[i].Start.strftime('%Y-%m-%d %H:%M') + "\n")
            f.write("\tEnd="+ sche[i].End.strftime('%Y-%m-%d %H:%M') + "\n")
...
# EAMS = EAMS()
# 
# EAMS.readProblemInstance('Input/eams_LRLC_custom.cfg')
# EAMS.readProblemInstance('Input/eams_LRHC_custom.cfg')
# EAMS.readProblemInstance('Input/eams_HRLC_custom.cfg')
# EAMS.readProblemInstance('Input/eams_HRHC_custom.cfg')
# # EAMS.readProblemInstance('Input/eams_LRLC_custom.cfg')
#  
# solver = Solver(EAMS)
# # solver.presolve()
